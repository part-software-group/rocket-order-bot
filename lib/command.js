...
const command = [
  ...
  {
    name: 'del_daily_menu_date',
    command: /^\s*!del_daily_menu_date\s+([ps])\s+([0-9]{4}-[0-9]{2}-[0-9]{2}|[0-9]{4}.[0-9]{2}.[0-9]{2}|[0-9]{4}\/[0-9]{2}\/[0-9]{2}|[0-9]{8}|[0-9]{2}|all)$/,
    info: 'حذف ناهار برای یک تاریخ خاص',
    sample: '!del_daily_menu_date <p|s> <date|all>',
    secure: true,
    show: true,
  },
  {
    name: 'accept_order',
    command: /^\s*!accept_order\s(no|pick)\s([a-z0-9-]+)\s([0-9]+)\s([1-9]+)/,
    info: 'تایید سفارش',
    sample: '!accept_order <no|pick> <token> <lunch> <count>',
    secure: false,
    show: false,
  },
  {
