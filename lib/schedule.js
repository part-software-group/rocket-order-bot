...
module.exports = function(sqlite) {
  ...
  logger.info(
    `Schedule set to every day ${startHour.toString().padStart(2, '0')}:${startMinute.toString().padStart(2, '0')}`,
  );
  schedule.scheduleJob(`${startMinute} ${startHour} * * *`, startRequest.bind(null, sqlite));

  for (let i = 0; i < timer.length; i++) {
    logger.info(
      `Schedule set to every day ${timer[i].hour.toString().padStart(2, '0')}:${timer[i].minute
        .toString()
        .padStart(2, '0')}`,
    );
  ...
  const finishDate = new persianDate([null, null, null, Number(startHour), Number(startMinute)])
    .add('minute', diff * attempt)
    .add('minute', 3);
  logger.info(
    `Schedule set to every day ${finishDate
      .hour()
      .toString()
...
function startRequest(sqlite) {
  ...
  Promise.all([
    ...
    sqlite.all(`SELECT id, list FROM lunch_list WHERE order_date = ? AND delete_date = 0`, [`${week}${nextDay}`]),
