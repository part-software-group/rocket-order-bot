...
module.exports = function() {
  ...
  schedule.scheduleJob(`${startMinute} ${startHour} * * *`, startRequest.bind(null, db));
...
function attemptRequest() {
  db.PersonOrder.findScheduleOrder()
    .then((data) => {
      const list = data.filter((v) => v.personOrderMenu.length === 0);
    ...
    .then(() => startRequest())
    .catch((error) =>
      logger.error(`Can't attempt lunch request! ${error.message.toString()}`),
    );
}
