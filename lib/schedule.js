...
module.exports = function(sqlite) {
  ...
  schedule.scheduleJob(`${startMinute} ${startHour} * * *`, updateOrderProcessFinish.bind(null, sqlite, finishDate));
};
...
function updateOrderProcessFinish(sqlite, finishDate) {
  const orderUnixFinish = new persianDate()
    .hour(finishDate.hour())
    .minute(finishDate.minute())
    .unix();
  sqlite
    .run(`INSERT OR REPLACE INTO settings VALUES (?, ?)`, ['order_process_finish', orderUnixFinish.toString()])
    .then(() => true)
