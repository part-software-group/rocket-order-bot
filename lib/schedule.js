...
module.exports = function() {
  ...
  logger.info(
    `Schedule start order set to every day ${startHour
      .toString()
  ...
  for (let i = 0; i < timer.length; i++) {
    logger.info(
      `Schedule attempt order set to every day ${timer[i].hour
        .toString()
  ...
  const finishDate = new persianDate()
    ...
    .add('minute', 3);
  logger.info(
    `Schedule finish order set to every day ${finishDate
      .hour()
      .toString()
...
async function removeOrder(PersonOrder) {
