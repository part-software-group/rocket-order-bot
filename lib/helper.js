...
function nextDateInfo() {
  ...
  let date = new persianDate([1397, 8, 9]);
  let DateOfWeek = null;
  ...
  function nextDay() {
    ...
    nextDayIndex = Number(date.format('d')) - 1;
    if (nextDayIndex >= 4) DateOfWeek = date.add('days', 7);
    else DateOfWeek = date;

    if (days.indexOf(name) === -1) return nextDay();
    if (holiday.indexOf(Number(date.format('YYYYMMDD'))) !== -1) return nextDay();
  }

  DateOfWeek.formatPersian = false;
  date.formatPersian = false;

  const week = DateOfWeek.format('w') % 2 === 0 ? 2 : 1;
