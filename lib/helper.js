    ...
    case 'help': {
      ...
      for (let i = 0; i < commands.length; i++) {
        ...
        body.attachments[j].fields.push({
          ...
          short: typeof args[0][commands[i]].short === 'boolean' ? args[0][commands[i]].short : true,
        });
      }
      break;
    }
        ...
        return sendRocketWarning('lunch_next_again_process', username, [
          {
            ...
            value: new persianDate.unix(Number(data[0].value)).format('HH:mm:ss YYYY-MM-DD'),
          },
        ]);
...
async function updateOrderProcessFinish(sqlite, finishDate) {
  const orderUnixFinish = new persianDate()
    .hour(finishDate.hour())
    .minute(finishDate.minute())
    .unix();

  return sqlite.run(`INSERT OR REPLACE INTO settings VALUES (?, ?)`, [
    'order_process_finish',
    orderUnixFinish.toString(),
  ]);
}
