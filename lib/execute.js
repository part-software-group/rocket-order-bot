...
function getDailyMenu(sqlite, username, func, type) {
  const isPrimary = type === 'p' ? '1' : '0';
  ...
  sqlite
    ...
    .then((data) => helper.sendRocketSuccess(func, username, [data]))
    .catch((error) =>
      helper.sendRocketFail('error', username, [
        {
          key: 'code',
...
function getDailyMenuDate(sqlite, username, func, type, pick) {
  const isPrimary = type === 'p' ? '1' : '0';
  let selectDate = pick.replace(/[^0-9]+/g, '');

  if (pick.replace(/\s+/g, '') === 'all') selectDate = 0;
  else if (selectDate > 999) selectDate = helper.convertDateToPersian(selectDate).format('YYYYMMDD');

  sqlite
    .all(
      `SELECT d.order_date order_date, group_concat(m.name, '|') list FROM daily d, daily_menu dm, menu m WHERE d.id = dm.daily_id AND dm.menu_id = m.id AND d.is_primary = ? AND d.order_date = ? AND d.delete_date = 0 AND dm.delete_date = 0 AND m.delete_date = 0 GROUP BY d.id`,
      [isPrimary, selectDate],
    )
    .then((data) => helper.sendRocketSuccess(func, username, [data]))
    .catch((error) =>
      helper.sendRocketFail('error', username, [
...
function setDailyMenuDate(sqlite, username, func, type, pick, count, list) {
  const isPrimary = type === 'p' ? '1' : '0';
  ...
  sqlite
    ...
    .then(() =>
      sqlite.run(`INSERT INTO daily (order_date, max_count, is_primary, insert_date) VALUES (?, ?, ?, ?)`, [
        selectDate,
        count,
        isPrimary,
        helper.getDate(),
    ...
    .then(() => sqlite.run(`COMMIT`))
    .then(() => helper.sendRocketSuccess(func, username))
    .catch((error) =>
      Promise.all([
        sqlite.run('ROLLBACK'),
        helper.sendRocketFail('error', username, [
...
function delDailyMenuDate(sqlite, username, func, type, pick) {
  const deleteDate = helper.getDate();
  const isPrimary = type === 'p' ? '1' : '0';
  let selectDate = pick.replace(/[^0-9]+/g, '');
  let dailyId;
  ...
  sqlite
    .all(`SELECT id FROM daily WHERE order_date = ? AND is_primary = ? AND delete_date = 0`, [selectDate, isPrimary])
    .then(async (data) => {
      if (!data.length) {
        await helper.sendRocketWarning(func, username);
    ...
    .then(() => sqlite.run(`UPDATE daily SET delete_date = ? WHERE id = ?`, [deleteDate, dailyId]))
    .then(() => sqlite.run(`UPDATE daily_menu SET delete_date = ? WHERE daily_id = ?`, [deleteDate, dailyId]))
    .then(() => sqlite.run(`COMMIT`))
    .then(() => helper.sendRocketSuccess(func, username))
    .catch((error) => {
...
function acceptOrder(sqlite, username, func, type, oid, mid, channelId, messageId) {
  let rocketMessageId;
  let rocketRoomId;
  ...
  sqlite
    ...
    .then((data) => {
       ...
       * @property username
       */
      if (!data.length) return helper.sendRocketFail('accept_order', username);
      else if (data.length && data[0].username !== username) return helper.sendRocketFail('no_permission', username);
      else if (data.length && data[0].lunch) return helper.sendRocketWarning('accept_order', username);
    ...
    .then((data) => {
      if (!data) return;
      else if (!data.length) return helper.sendRocketFail('accept_order', username);
    ...
    .then((data) => {
      if (data) {
        ...
        return helper.sendRocketSuccess('accept_order', username, [lunch, oid, property.format('DD-MM-YYYY')]);
      }
...
function againLunchNext(sqlite, username, count = 1, change = false) {
  ...
  function run(attempt, newUserOrder) {
    ...
    sqlite
      ...
      .then(() =>
        Promise.map(orderInfo, (v) => sendNewLunchAgain(v.id, v.person, v.listWeek, v.listToday, attempt === 0 ? change : false)),
      )
...
function changeCurrentOrders(sqlite, hour, minute, lunchList, username) {
  ...
  sqlite
    ...
    .then(() => sqlite.run('COMMIT'))
    .then(() => {
      setTimeout(() => againLunchNext(sqlite, username, attempt, true), 300);
