...
function getLunchList(sqlite, username) {
  sqlite
...
function getLunchListDate(sqlite, selectDate, username) {
  sqlite
...
function addLunchListDate(sqlite, selectDate, selectList, username) {
  sqlite
...
function againLunchNext(sqlite, username, count = 1) {
  ...
  Promise.all([
    sqlite.all(
      `SELECT o.lunch_list_id, l.list FROM lunch_order o, lunch_list l WHERE o.lunch_list_id = l.id AND o.insert_date / 1000000000 = ? AND o.delete_date = 0 limit 1`,
      [insertDate],
    ),
    sqlite.all(
      `SELECT id, username FROM person WHERE delete_date = 0 EXCEPT SELECT p.id, p.username FROM lunch_order o, person p WHERE o.person_id = p.id AND o.insert_date / 1000000000 = ? AND p.delete_date = 0`,
      [insertDate],
    ),
  ])
    .then(([lunch, users]) => {
      ...
      for (let i = 0; i < users.length; i++)
        newUserOrder.push({
          id: '',
          person: { id: users[i].id, username: users[i].username },
          list: [{ id: lunch[0].lunch_list_id, list: lunch[0].list }],
        });
  ...
  function run(attempt, newUserOrder) {
    ...
    sqlite
      ...
      .then(() => Promise.map(orderInfo, (v) => sendNewLunchAgain(v.id, v.person, v.list)))
      .then(() => helper.updateOrderProcessFinish(sqlite, finishDate))
      .then(() => sqlite.run(`COMMIT`))
...
function removeUser(sqlite, uid, username) {
