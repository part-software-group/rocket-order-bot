...
function againLunchNext(sqlite, username) {
...
function changeCurrentOrders(sqlite, hour, minute, lunchList, username) {
  const attempt = config.get('custom.order.attempt');
  const insertDate = Number(helper.getDate().substr(0, 8));
  ...
  sqlite
    ...
    .then((data) => Promise.all([data.lastID, sqlite.all(`SELECT id, username FROM person WHERE delete_date = 0`)]))
    .then(([lunchListId, person]) => Promise.all([lunchListId, person, sqlite.run('COMMIT')]))
    .then(([lunchListId, person]) => {
      for (let i = 0; i < attempt; i++)
        setTimeout(() => {
          if (i > 0) return againLunchNext(sqlite);

          person.map((v) => helper.sendLunchRequest(sqlite, v, [], [{ id: lunchListId, list: lunchList }], true));
        }, i * 3000);
